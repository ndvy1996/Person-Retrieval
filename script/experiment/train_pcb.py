...
def main():
  ...
  def test(load_model_weight=False):
    if load_model_weight:
      if cfg.model_weight_file != '':
          map_location = (lambda storage, loc: storage)
          checkpoint = torch.load(cfg.model_weight_file, map_location=map_location)

            # Check if 'state_dict' exists in the checkpoint
          if 'state_dict' in checkpoint:
              sd = checkpoint['state_dict']
          else:
              # If state_dict doesn't exist, assume the whole checkpoint is the state_dict
              sd = checkpoint

              # Manually filter out any unwanted keys (like 'scores', 'ep', 'state_dicts')
              sd_filtered = {k: v for k, v in sd.items() if k in model.state_dict()}

            # Load the filtered state dict into the model
              load_state_dict(model, sd_filtered)
              print('Loaded model weights from {}'.format(cfg.model_weight_file))
      else:
          load_ckpt(modules_optims, cfg.ckpt_file)
  ...
  for ep in range(start_ep, cfg.total_epochs):
    ...
    while not epoch_done:
      ...
      # Stack the losses
      losses = torch.stack([criterion(logits, labels_var) for logits in logits_list])

      # Aggregate the losses into a single scalar (e.g., sum or mean)
      loss = losses.mean()  # or losses.sum()
