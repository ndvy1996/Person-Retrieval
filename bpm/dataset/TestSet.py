...
class TestSet(Dataset):
  ...
  def __init__(
    ...
    # The im dir of all images
      self.im_dir = im_dir.decode('utf-8') if isinstance(im_dir, bytes) else im_dir
      self.im_names = [name.decode('utf-8') if isinstance(name, bytes) else name for name in im_names]
      self.marks = marks
  ...
  def eval(
    ...
    if any(mq_inds):
      ...
      #   )
      with measure_time('Multi Query, Computing scores...', verbose=verbose):
        zipped_keys = list(zip(*keys))  # Convert zip object to a list
        mq_mAP, mq_cmc_scores = compute_score(
          mq_g_dist,
          query_ids=np.array(zipped_keys[0]),  # Access the first element of the zipped list
          gallery_ids=ids[g_inds],
          query_cams=np.array(zipped_keys[1]),  # Access the second element of the zipped list
          gallery_cams=cams[g_inds]
        )
    ...
    if to_re_rank:
      ...
      if any(mq_inds):
        ...
        # Convert zip(*keys) to a list to make it subscriptable
          keys_list = list(zip(*keys))
    
          mq_mAP, mq_cmc_scores = compute_score(
            re_r_mq_g_dist,
            query_ids=np.array(keys_list[0]),  # Access the first element (query ids)
            gallery_ids=ids[g_inds],
            query_cams=np.array(keys_list[1]),  # Access the second element (query cams)
            gallery_cams=cams[g_inds]
          )
